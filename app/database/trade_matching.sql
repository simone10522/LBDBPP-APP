...
BEGIN
  FOR rec1 IN SELECT * FROM trade_cards LOOP
    -- Consider only rec2 with a higher user_id to avoid duplicates inversion
    FOR rec2 IN SELECT * FROM trade_cards WHERE user_id > rec1.user_id LOOP
      ...
      AND EXISTS (
        ...
        WHERE w2->>'id' = h1->>'id'
          ...
          WHERE h2->>'id' IN (
            SELECT w1->>'id' FROM jsonb_array_elements(rec1.what_i_want) AS w1
          );
          ...
          WHERE h1->>'id' IN (
            SELECT w2->>'id' FROM jsonb_array_elements(rec2.what_i_want) AS w2
          );

          -- Verifica che non esista già un trade con le stesse carte per gli stessi giocatori
          SELECT EXISTS(
            SELECT 1 FROM trade_matches 
            WHERE user1_id = rec1.user_id 
              AND user2_id = rec2.user_id
              AND user1_cards = candidate_user1_cards
              AND user2_cards = candidate_user2_cards
              AND status IN ('request sent', 'confirmed', 'completed')
          ) INTO exists_match;
